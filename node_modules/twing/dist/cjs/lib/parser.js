...
const wrapper_1 = require("./node/wrapper");
...
const createParser = (unaryOperators, binaryOperators, additionalTagHandlers, visitors, filters, functions, tests, options) => {
    ...
    const filterChildBodyNode = (stream, node, nested = false) => {
        ...
        if (testedNode.type === "text" && !(0, is_made_of_whitespace_only_1.isMadeOfWhitespaceOnly)(testedNode.attributes.data)) {
            ...
            throw (0, parsing_1.createParsingError)(`A template that extends another one cannot include content outside Twig blocks. Did you forget to put the content inside a {% block %} tag?`, node, stream.source.name);
        }
        const { type } = node;
        ...
        if (nested && (type === "block_reference")) {
            if (level >= 3) {
                throw (0, parsing_1.createParsingError)(`A block definition cannot be nested under non-capturing nodes.`, node, stream.source.name);
            }
            else {
    ...
    const getFilterExpressionFactory = (stream, name, line, column) => {
        ...
        if (filter) {
            ...
            const error = (0, parsing_1.createParsingError)(`Unknown filter "${name}".`, { line, column }, stream.source.name);
    ...
    const getFunctionExpressionFactory = (stream, name, line, column) => {
        ...
        if (twingFunction) {
            ...
            const error = (0, parsing_1.createParsingError)(`Unknown function "${name}".`, { line, column }, stream.source.name);
    ...
    const getFunctionNode = (stream, name, line, column) => {
        switch (name) {
            ...
            case 'block':
                ...
                const keyValuePairs = (0, array_1.getKeyValuePairs)(blockArgs);
            ...
            case 'attribute':
                ...
                const attributeKeyValuePairs = (0, array_1.getKeyValuePairs)(attributeArgs);
    ...
    const getPrimary = (stream) => {
        ...
        if ((operator = isUnary(token)) !== null) {
            ...
            return parsePostfixExpression(stream, expressionFactory([expression, (0, node_1.createBaseNode)(null)], token.line, token.column), token);
        }
    ...
    const getTestName = (stream) => {
        ...
        const error = (0, parsing_1.createParsingError)(`Unknown test "${name}".`, { line, column }, stream.source.name);
    ...
    const parse = (stream, tag = null, test = null) => {
        ...
        try {
            ...
            if (parent !== null && (body = filterChildBodyNode(stream, body)) === null) {
                body = (0, node_1.createBaseNode)(null);
            }
        }
        catch (error) {
            if (!error.source) {
                error.source = stream.source.name;
            }
            throw error;
        }
        let node = (0, template_1.createTemplateNode)((0, body_1.createBodyNode)(body, 1, 1), parent, (0, node_1.createBaseNode)(null, {}, blocks), (0, node_1.createBaseNode)(null, {}, macros), (0, node_1.createBaseNode)(null, {}, traits), embeddedTemplates, stream.source, 1, 1);
        // passed visitors
    ...
    const parseArguments = (stream, namedArguments = false, definition = false, allowArrow) => {
        ...
        while (!stream.test("PUNCTUATION", ')')) {
            ...
            if (namedArguments && (token = stream.nextIf("OPERATOR", '='))) {
                if (value.type !== "name") {
                    throw (0, parsing_1.createParsingError)(`A parameter name must be a string, "${value.type.toString()}" given.`, value, stream.source.name);
                }
                ...
                if (definition) {
                    ...
                    if (notConstantNode !== null) {
                        throw (0, parsing_1.createParsingError)(`A default value for an argument must be a constant (a boolean, a string, a number, or an array).`, notConstantNode, stream.source.name);
                    }
                }
                else {
                    value = parseExpression(stream, 0, allowArrow);
    ...
    const parseAssignmentExpression = (stream) => {
        ...
        while (true) {
            ...
            if (['true', 'false', 'none', 'null'].indexOf(value.toLowerCase()) > -1) {
                throw (0, parsing_1.createParsingError)(`You cannot assign a value to "${value}".`, token, stream.source.name);
            }
        ...
        return (0, wrapper_1.createWrapperNode)(targets, line, column);
    };
    const parseArrow = (stream) => {
        ...
        if (stream.look(1).test("ARROW")) {
            ...
            stream.expect("ARROW");
            return (0, arrow_function_1.createArrowFunctionNode)(parseExpression(stream, 0), (0, node_1.createBaseNode)(null, {}, names), line, column);
        }
        ...
        while (true) {
            ...
            if (!token.test("NAME")) {
                throw (0, parsing_1.createParsingError)(`Unexpected token "${(0, lexer_1.typeToEnglish)(token.type)}" of value "${token.value}".`, token, stream.source.name);
            }
    ...
    const parseHashExpression = (stream) => {
        ...
        while (!stream.test("PUNCTUATION", '}')) {
            ...
            if (stream.test("SPREAD_OPERATOR")) {
                ...
                const spreadNode = (0, spread_1.createSpreadNode)(expression, current.line, current.column);
                elements.push({
                    key: (0, node_1.createBaseNode)(null),
                    value: spreadNode
                });
                continue;
            }
        ...
        switch (token.type) {
            ...
            case "OPERATOR":
                ...
                if (match !== null && match[0] === token.value) {
                    ...
                    const { expressionFactory } = operator;
                    node = expressionFactory([expression, (0, node_1.createBaseNode)(null)], token.line, token.column);
                    break;
                }
            default:
                if (token.test("PUNCTUATION", '[')) {
                    ...
                    throw (0, parsing_1.createParsingError)(`Unexpected operator of value "${token.value}". Did you try to use "===" or "!==" for strict comparison? Use "is same as(value)" instead.`, token, stream.source.name);
                }
                else {
                    throw (0, parsing_1.createParsingError)(`Unexpected token "${(0, lexer_1.typeToEnglish)(token.type)}" of value "${token.value}".`, token, stream.source.name);
                }
        ...
        if (token.value === '.') {
            ...
            if ((token.type === "NAME") || (token.type === "NUMBER") || (token.type === "OPERATOR" && (match !== null))) {
                ...
                if (stream.test("PUNCTUATION", '(')) {
                    type = "method";
                    const argumentsNode = parseArguments(stream);
                    for (const { value } of (0, array_1.getKeyValuePairs)(argumentsNode)) {
                        elements.push(value);
                    }
                }
            }
            else {
                throw (0, parsing_1.createParsingError)('Expected name or number.', { line, column: column + 1 }, stream.source.name);
            }
        ...
        while (!stream.isEOF()) {
            switch (stream.current.type) {
                ...
                case "TAG_START":
                    ...
                    token = stream.current;
                    if (token.type !== "NAME") {
                        throw (0, parsing_1.createParsingError)('A block must start with a tag name.', token, stream.source.name);
                    }
                    if ((test !== null) && test(token)) {
                        if (Object.keys(children).length === 1) {
                            return children[0];
                        }
                        return (0, wrapper_1.createWrapperNode)(children, line, column);
                    }
                    if (!tokenParsers.has(token.value)) {
                        let error;
                        if (test !== null) {
                            error = (0, parsing_1.createParsingError)(`Unexpected "${token.value}" tag`, token, stream.source.name);
                            error.appendMessage(` (expecting closing tag for the "${tag}" tag defined line ${line}).`);
                        }
                        else {
                            error = (0, parsing_1.createParsingError)(`Unknown "${token.value}" tag.`, token, stream.source.name);
